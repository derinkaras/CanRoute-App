...
const CanModal = ({
    ...
    const confirmServiceLog = async () => {
        ...
        if (logExists) {
            ...
            const updatedLogs = [
                ...serviceLogsOfWeek.filter(log => !(log.canId === can._id && log.servicedDate === todayDate)),
                newServiceData
            ];
            ...
            await addUserServiceLog(newServiceData);
    ...
    const handleOpenMap = () => {
        if (can) openMaps(can.location.latitude, can.location.longitude, can.label);
    };

    const handleServiceStatusChange = (status: string) => {
        switch (status) {
            case 'Serviced':
                setCurrentCanServiceStatus('serviced'); break;
            case 'Not Needed':
                setCurrentCanServiceStatus('not_needed'); break;
            case 'Unserviced':
                setCurrentCanServiceStatus('unserviced'); break;
        }
    };
        ...
        >
            <View className="flex-1 relative justify-center items-center">
                ...
                <View className="bg-white w-96 h-auto z-5 rounded-2xl p-6 items-center gap-5">
                    {showQrModal ? (
                        <>
                            <Text className="text-darkBlue text-xl font-bold text-center">Scan to Access Can</Text>
                            <QRCode
                                value={can.qrCode}
                                size={200}
                            />
                            ...
                            >
                                <Text className="text-white text-base font-semibold">Close QR Code</Text>
                            </TouchableOpacity>
                        </>
                    ) : (
                        <>
                            ...
                            <View className="flex-row gap-4">
                                <TouchableOpacity onPress={handleOpenMap} className="items-center gap-2" activeOpacity={0.8}>
                                    <Image source={icons.route} className="size-8" resizeMode="contain" tintColor="#193a5a" />
                                </TouchableOpacity>
                                <TouchableOpacity onPress={handleDirections} className="items-center gap-2" activeOpacity={0.8}>
                                    <Image source={icons.search} className="size-8" resizeMode="contain" tintColor="#193a5a" />
                                </TouchableOpacity>
                                <TouchableOpacity onPress={() => setShowQrModal(true)} className="items-center gap-2" activeOpacity={0.8}>
                                    <Image source={icons.qrcode} className="size-8" resizeMode="contain" tintColor="#193a5a" />
                                </TouchableOpacity>
                            </View>
                            ...
                            <View className={`mt-3 px-4 py-2 rounded-full ${getStatusColor(currentCanServiceStatus)}`}>
                                <Text className="text-white font-semibold text-sm">
                                    ...
                                    currentCanServiceStatus === "not_needed" ? "Not Needed" : "Unserviced"}
                                </Text>
                            </View>
                            <View className="flex-row items-center gap-4 mt-2">
                                <Text className="text-base font-semibold text-darkBlue">Illegal Dumping?</Text>
                                <Switch
                            ...
                            <View className="flex-row justify-between gap-2 mt-4">
                                <TouchableOpacity className="bg-green-600 px-4 py-2 rounded-2xl" onPress={() => handleServiceStatusChange('Serviced')}>
                                    <Text className="text-base text-white">Serviced</Text>
                                </TouchableOpacity>
                                <TouchableOpacity className="bg-orange-500 px-4 py-2 rounded-2xl" onPress={() => handleServiceStatusChange('Not Needed')}>
                                    <Text className="text-base text-white">Not needed</Text>
                                </TouchableOpacity>
                                <TouchableOpacity className="bg-red-500 px-4 py-2 rounded-2xl" onPress={() => handleServiceStatusChange('Unserviced')}>
                            ...
                            >
                                ...
                                <Image source={icons.check} resizeMode="contain" className="w-6 h-6" tintColor="#fff" />
                            </TouchableOpacity>
                        </>
                    )}
                </View>
